...
const handler = async (m, {conn, text, usedPrefix, command, args}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw '*[❗𝐈𝐍𝐅𝐎❗] OS 𝙲𝙾𝙼𝙰𝙽𝙳𝙾𝚂 +𝟷𝟾 ESTÃO DESATIVADOS NESSE GRUPO,SE É 𝙰𝙳𝙼𝙸𝙽 E DESEJA 𝙰𝚃𝙸𝚅𝙰𝙻𝙾𝚂 𝚄𝚂𝙴 O 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 #enable modohorny*';
  if (!text) throw `*[❗] INSIRA O 𝙽𝙾M𝙴 𝙳𝙴 𝙰𝙻𝙶𝚄M𝙰 𝙲𝙰𝚃𝙴𝙶𝙾𝚁𝙸𝙰 𝙳𝙴 𝙷𝙴𝙽𝚃𝙰𝙸, 𝙴X𝙴𝙼𝙿𝙻𝙾: ${usedPrefix + command} miku*`;
  try {
    m.reply(global.wait);
    ...
    throw `*[❗] 𝙴𝚁𝚁𝙾𝚁, PESQUISE OUTRA CATEGORIA*`;
  }
};
